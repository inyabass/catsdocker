#!/usr/bin/env bash
. library.sh
docker exec -it $KAFKA_CONTAINER_NAME kafka-topics.sh --create --zookeeper $HOST_MACHINE:2181 --replication-factor 1 --partitions 10 --topic test-request
docker exec -it $KAFKA_CONTAINER_NAME kafka-topics.sh --create --zookeeper $HOST_MACHINE:2181 --replication-factor 1 --partitions 10 --topic test-response
sleep 2
